(function () {
    ...
    app.controller(controllerId, [
        '$scope', '$rootScope', '$state', '$stateParams', 'requestProfileService', 'requestService', 'modalService','requestIdService',
        function ($scope, $rootScope, $state, $stateParams, requestProfileService, requestService, modalService, requestIdService) {
            ...
            vm.openPersonalObjectDictionary = function () {
                modalService.showModal({
                    templateUrl: "/AngularTemplate/PersonalObjects",
                    ...
                    modal.close.then(function (result) {
                        ...
                        if (result.cancel)
                            return;
                        vm.request.ObjectId = result.personalObject.Id;
                        vm.request.ObjectName = result.personalObject.Name;

                    });
                });
            }

            vm.getListPersonalObject = function (name, response) {

                if (name == 'undefined' || name == 'null' || name == null)
                    return;

                requestProfileService.getListPersonalObjectByName(name).then(function (results) {
            ...
            vm.selectPersonalObject = function (p) {
                vm.request.ObjectId = p.ObjectId;
            };
            
            vm.goToRequestProfile = function () {
                $state.go("requestProfile");
            }
                        
            vm.goToPersonalProfile = function () {
                $state.go("personalProfile");
            }
